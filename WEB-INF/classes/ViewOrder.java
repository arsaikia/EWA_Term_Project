...
protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(request.getParameter("Order")!=null && request.getParameter("Order").equals("CancelOrder"))
		{
			if(request.getParameter("orderName") != null)
			{
				...
				{
					System.out.println("\nException in View Order -> CancelOrder!!");
				...
				for (OrderPayment oi : orderPayments.get(orderId)) 
					{
						Date orderedDate = oi.getOrderDate();
						Date deliveryDate = oi.getDeliveryDate();
						Date currentDate = new Date();
						long difference_In_Time = deliveryDate.getTime() - orderedDate.getTime();
  						long difference_In_Days = (difference_In_Time / (1000 * 60 * 60 * 24)) % 365; 
												
						pw.print("</br></br><h3 style='color:gainsboro;'>Order date: " + (orderedDate) + "</h3></br>");
						pw.print("<h3 style='color:gainsboro;'>Expected delivery: " + (deliveryDate) + "</h3></br></br></br>");

						// A Manager (Salseman) Can cancel the order whenever !
						String usertype = utility.usertype();
						Boolean isUserSalseman = usertype.equals("manager");
						Boolean isCurrentOrder = oi.getOrderName().equals(orderName);
						Boolean isAccountOwner = oi.getUserName().equals(username);

						if((difference_In_Days < 5) && !isUserSalseman){
							pw.print("<h3 id='warning'>Because your delivery/ pickup is scheduled withing 5 upcoming days, you CANNOT cancel your orer now.</h3>");
						}
						else if(isCurrentOrder && (isAccountOwner || isUserSalseman) )
						{
							ListOrderPayment.add(oi);
							pw.print("<h3 id='success'>Your Order has been successfully Cancelled.</h3>");								
						}
					}
